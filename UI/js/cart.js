...
function appendtoTable(cellArr, tr, tableName) {
  cellArr.forEach((cell) => {
    // append each cell to td then to tr
...
setTimeout(() => {
  ...
  [...cartBtns].forEach((cartBtn) => {
    cartBtn.addEventListener('click', () => {
      if (
        !document.getElementById('menuWelcome').textContent.includes('Welcome ')
      ) {
        // open modal asking user to sign up
        msg.innerHTML =
          'Por favor <a href="/signup"><b>cadastre-se</b></a> ou <a href="/login"><b>login</b></a> para continuar com seu pedido!';
      ...
      let quantity = Number(
        document.querySelector(`select#selectQty${uniqueId}`).value,
      );
      ...
      // Open a modal
      msg.innerHTML = `<b>${qty}x ${name}</b> Adicionado ao carrinho com sucesso!`;
      displayModal(generalModal, span1);
...
checkoutBtn.onclick = () => {
  if (!address.value || address.value === 'null') {
    cartErr.innerHTML = 'Por favor, preencha seu endereço de entrega';
    return;
  }
    ...
    fetch(req)
      .then((resp) => {
        resp
          ...
          .then((res) => {
            if (res.error) {
              msg.innerHTML = `<p style="color: red">Error: ${
          ...
          <br>Nós vamos entrar em contato com você no telefone <b>${phone.value}</b> or <b>${localStorage.email}</b> para te passar mais detalhes.
          <h6 style="color: red"><i> Por favor, verifique o id correspondente ao seu pedido.</i></h6>`;
