...
export default (orderC, userC, menuC) => {
  ...
  server.delete(`${prefix}/orders/:orderId`, (req, res) =>
    orderC
  ...
  // ****** USER ROUTES **** //
  // SIGNUP /user
  server.post(`${prefix}/auth/signup`, (req, res) =>
    userC
      .create(req)
      .then((result) => res.status(result.statusCode).json(result))
      .catch((err) =>
  ...
  server.get(`${prefix}/users/:userId/orders`, AuthC.verifyToken, (req, res) =>
    userC
      .findOrdersByUserId(req)
      .then((result) => res.status(result.statusCode).json(result))
      .catch((err) =>
        res
          .status(err.statusCode || 500)
  ...
  server.get(`${prefix}/users`, AuthC.verifyAdminToken, (_req, res) =>
    userC
      .read()
      .then((result) => res.status(result.statusCode).json(result))
      .catch((err) =>
  ...
  // ****** MENU ROUTES **** //
  server.get(`${prefix}/menu`, AuthC.verifyToken, (_req, res) =>
    menuC
      .read()
      .then((result) => res.status(result.statusCode).json(result))
  ...
  server.delete(`${prefix}/menu/:foodId`, AuthC.verifyAdminToken, (req, res) =>
    menuC
      .delete(req)
      .then((result) => res.status(result.statusCode).json(result))
      .catch((err) => res.status(err.statusCode || 500).json(err)),
  ...
  server.get('/api/v1', (req, res) =>
    res
      .status(200)
      .json({ message: 'Welcome to API version 1 of FastFoodFast' }),
  );
  ...
  server.get('*', (req, res) =>
    res
      ...
      .json({
        ...
        error: 'Sorry, this route is not available',
      }),
  );
  server.post('*', (req, res) =>
    res
